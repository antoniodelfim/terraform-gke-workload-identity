/**
 *  # SRE App - Workload Identity Federation
 *
 * Configuração de Workload Identity Federation para a aplicação SRE App.
 * Este arquivo configura a KSA e GSA com permissões para acesso a buckets do Storage.
...
locals {
  ...
  app_id    = "app"  # Nome da aplicação sem o prefixo do namespace
...
locals {
  ...
  roles_to_add = [
    ...
    "roles/storage.admin",           # Controle total do bucket e objetos
    ...
    "roles/pubsub.admin",            # Controle total do PubSub
  ...
  roles_to_remove = [
    ...
    # "roles/pubsub.publisher",        # Remover acesso de publicação ao PubSub
...
module "workload_identity" {
  ...
  description  = "Gerencia acesso ao Storage para a aplicação ${local.app_name}"
